...
class writer_configuration {
 ...
 public:
  ...
  bool pretty_printing() const { return m_pretty_printing; }
  ...
  size_t indent_spaces() const { return m_indent_spaces; }
  ...
  bool use_tabs() const { return m_use_tabs; }
...
class buffered_writer {
  static_assert(Size != 0, "Illegal instantiation of buffered_writer");
 ...
 public:
  ...
  buffered_writer& operator<<(const char (&str)[N]) {
    static_assert(N != 0,
        "Illegal instantiation of buffered_writer::operator<<(const char "
        "(&str)[N])");

    for (size_t i = 0; i < N - 1; i++) { operator<<(str[i]); }
...
class writer {
 ...
 public:
  ...
  void close() {
    ...
    if (m_status == EMPTY) { write_opening_bracket(); }
...
class array_writer : public writer {
 public:
  ...
  void write(const V& value) {
    write_helper(nullptr, value, default_value_writer<V>());
  }

  template <typename V, typename ValueWriter>
  void write(const V& value, const ValueWriter& value_writer) {
    write_helper(nullptr, value, value_writer);
  }
...
struct default_value_writer<null_t> {
  void operator()(std::ostream& stream, null_t) const { stream << "null"; }
};
...
void write_array(std::ostream& stream,
  ...
  for (InputIt it = begin; it != end; ++it) { writer.write(*it, value_writer); }
